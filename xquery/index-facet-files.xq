    ...
    for $file in $files
      ...
      for $terms in $doc//controlaccess/*[local-name()=$local_names]
        ...
        group by $text, $ark, $db_id, $facet_db
          where not($text="")
            let $existing_term := db:get($facet_db)/terms[@db=$db_id]/term[@text=$text][1]
            where not($existing_term/ark[text()=$ark])
              return
                if (exists($existing_term)) then
                  insert node <ark>{$ark}</ark> as last into $existing_term
                else
                  insert node <term text="{$text}"><ark>{$ark}</ark></term> as last into db:get($facet_db)/terms[@db=$db_id]
}

    ...
    if ($mysqli = connect()) {
      $ark_query = 'SELECT arks.id, arks.ark, arks.file, arks.date, GREATEST(arks.date, COALESCE(max_updates.max_date, 0)) as last_date,
        arks.repo_id, arks.cached, arks.active FROM arks LEFT JOIN (
          SELECT ark, MAX(date) as max_date FROM updates WHERE ark="' . $ark . '"
        ) as max_updates
  ...
  function delete_cache() {
    ...
    if (!empty($bucket)) {
      ...
      try {
        ...
        $s3->put_source($this->get_qualifier() . '.html', $html);
      }
      catch (Exception $e) {
  ...
  function get_title() {
    if (!isset($this->title)) {
      try {
        ...
        $this->title = 'Unknown';
        // Mail webmaster
        $mail = new AW_Mail(ADMIN_EMAIL, 'Title Exception for ' . $this->get_ark(), $e->getMessage());
        $mail->send();
      }
    }
    return $this->title;
